...
import { snapshotFile } from "./utils/localFile";
...
const MemoEditorImpl: React.FC<MemoEditorProps> = ({
  ...
  const handleFiles = useCallback(
    async (files: File[], placement: { inline: false } | { inline: true; position?: number }) => {
      if (getState().ui.isLoading.saving || getState().ui.isLoading.uploading) return;
      dispatch(actions.setLoading("uploading", true));
      try {
        const snapshots: File[] = [];
        for (const file of files) snapshots.push((await snapshotFile(file)).file);
        const localFiles = toLocalFiles(snapshots, { createBlobUrl, saveMediaMetadata });
        ...
        if (getState().ui.pendingInlineImageInsertions === 0) dispatch(actions.setLoading("uploading", false));
      }
    },
    ...
    <>
      ...
      */}
      ...
      >
        ...
        {/* Diary Mode: for new memos the timestamp is surfaced through
            DiaryDateControl below, so only edit mode shows the popover here. */}
        {memoName && (

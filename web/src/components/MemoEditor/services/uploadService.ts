...
export const uploadService = {
  async uploadFile(localFile: LocalFile, signal?: AbortSignal): Promise<Attachment> {
    ...
    for (let offset = 0; ; ) {
      ...
      const request: Pick<UploadAttachmentRequest, "upload" | "writeOffset" | "data" | "finishWrite"> = {
        ...
        data: localFile.content?.slice(offset, end) ?? new Uint8Array(await file.slice(offset, end).arrayBuffer()),
  ...
  async uploadFiles(localFiles: LocalFile[]): Promise<Attachment[]> {
    ...
    // Keep bulk uploads sequential to avoid overlapping large image processing.
    for (const [index, localFile] of localFiles.entries()) {
      try {
        attachments.push(await this.uploadFile(localFile));
      } catch (error) {
        throw new Error(`Failed to upload attachment ${index + 1}/${localFiles.length} (${localFile.file.name})`, { cause: error });
      }
    }
    return attachments;
  },

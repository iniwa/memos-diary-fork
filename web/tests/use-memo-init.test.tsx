...
describe("useMemoInit", () => {
  ...
  it("restores boundary tags separately from the draft body and attachments", () => {
    const attachment = create(AttachmentSchema, { name: "attachments/diary" });
    cacheService.saveNow(cacheService.key("users/steven", "restored-draft"), "Diary body\n\n#photo #diary", [attachment]);
    render(
      <EditorProvider>
        <Probe />
      </EditorProvider>,
    );
    expect(getEditorState().content).toBe("Diary body");
    expect(getEditorState().tags).toEqual(["photo", "diary"]);

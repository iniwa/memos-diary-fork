...
describe("chunked attachment uploads", () => {
  ...
  it("uploads snapshot bytes without re-reading the original file", async () => {
    const local = { ...localFile("old"), content: new TextEncoder().encode("new") };
    const slice = vi.spyOn(local.file, "slice");
    await uploadService.uploadFile(local);
    expect(slice).not.toHaveBeenCalled();
    expect(new TextDecoder().decode(uploadAttachment.mock.calls[1][0].data)).toBe("new");
  });

  it("finishes each file before starting the next and stops on failure", async () => {
    const upload = vi.spyOn(uploadService, "uploadFile");
    let finishFirst!: (value: typeof attachment) => void;
    ...
    upload.mockRejectedValueOnce(new Error("failed"));
    const result = uploadService.uploadFiles([localFile("one"), localFile("two"), localFile("three")]);
    expect(upload).toHaveBeenCalledTimes(1);
    finishFirst(attachment);
    await expect(result).rejects.toThrow("Failed to upload attachment 2/3");

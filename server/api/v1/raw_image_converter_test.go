...
// is installed, then go test ./server/api/v1/... -run TestConvertRawImageToJPEG.
...
func TestUploadPipelineRejectsFailedRawConversion(t *testing.T) {
	t.Setenv(rawConversionEnabledEnv, "true")
	for _, chunked := range []bool{false, true} {
		t.Run(map[bool]string{false: "unary", true: "chunked"}[chunked], func(t *testing.T) {
			svc, ctx := newUploadTestService(t)
			input := []byte("not a RAW image")
			var err error
			if chunked {
				_, err = svc.UploadAttachment(ctx, &v1pb.UploadAttachmentRequest{Upload: uploadSpec("photo.nef", int64(len(input))), Data: input, FinishWrite: true})
			} else {
				_, err = svc.CreateAttachment(ctx, &v1pb.CreateAttachmentRequest{Attachment: &v1pb.Attachment{Filename: "photo.nef", Type: "application/octet-stream", Content: input}})
			}
			require.ErrorContains(t, err, "failed to convert RAW image")
			attachments, listErr := svc.Store.ListAttachments(ctx, &store.FindAttachment{})

...
const (
	// ThumbnailCacheFolder matches the file server cache directory.
...
func imageOptimizerConfigFromEnv() imageOptimizerConfig {
	return imageOptimizerConfig{
		...
		PreviewMaxEdge:   parseIntEnv(imageOptimizerPreviewMaxEdgeEnv, defaultPreviewMaxEdge, 1, imagelimit.MaxPixels),
		PreviewQuality:   parseIntEnv(imageOptimizerPreviewQualityEnv, defaultPreviewJPEGQuality, 1, 100),
		ThumbnailMaxEdge: parseIntEnv(ThumbnailMaxEdgeEnv, DefaultThumbnailMaxEdge, 1, imagelimit.MaxPixels),
...
func (s *APIV1Service) maybeOptimizeImageAttachment(ctx context.Context, attachment *store.Attachment) error {
	...
	if err != nil {
		...
		updateConvertedImageDimensions(attachment)
...
func decodeOptimizableImage(blob []byte) (image.Image, error) {
	if err := imagelimit.CheckReader(bytes.NewReader(blob)); err != nil {
		return nil, err
	}
...
// Preserve camera metadata while keeping display dimensions consistent with rewritten bytes.
func updateConvertedImageDimensions(attachment *store.Attachment) {
	metadata := attachment.Payload.GetMediaMetadata()
	if metadata == nil {
		return
	}
	config, _, err := image.DecodeConfig(bytes.NewReader(attachment.Blob))
	if err != nil {
		return
	}
